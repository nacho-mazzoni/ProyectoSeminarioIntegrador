  ...
  return (
    <>
      <div className="flex justify-between items-center mb-6">
        ...
        <Link href="/perfil/pedidos/nuevo"
          className="bg-amber-500 text-white px-4 py-2 rounded hover:bg-amber-600 text-sm">
          Nuevo Pedido
        </Link>
      </div>

      {pedidos.length === 0 && <p className="text-stone-400 text-center py-12">No tenes pedidos todavia</p>}

      <div className="flex flex-col gap-4">
        {pedidos.map((p) => {
          const ultimoEstado = p.historial?.[p.historial.length - 1]
          const esEditable = ultimoEstado?.estado === "PENDIENTE"
          const esCancelable = ultimoEstado?.estado === "PENDIENTE"
          return (
            <div key={p.idPedido} className="bg-white rounded-xl shadow-sm border p-5">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-lg font-semibold mt-1">Total: ${p.total.toFixed(2)}</p>
                  <p className="text-sm text-stone-500">
                    Entrega: {p.metodoEntrega} - {p.direccion}
                  </p>
                  {p.metodoPago && (
                    <p className="text-xs text-stone-400 mt-1">
                      Pago: {p.metodoPago === "mercado_pago" ? "Mercado Pago" : "Efectivo"}
                      {p.estadoPago && ` (${p.estadoPago})`}
                    </p>
                  )}
                </div>
                <div className="text-right flex flex-col gap-2 items-end">
                  <span className={`inline-block text-xs font-medium px-2.5 py-1 rounded-full ${
                    ultimoEstado?.estado === "CANCELADO" ? "bg-red-100 text-red-800"
                    : ultimoEstado?.estado === "ENTREGADO" ? "bg-green-100 text-green-800"
                    : "bg-amber-100 text-amber-800"
                  }`}>
                    {ultimoEstado?.estado ?? "PENDIENTE"}
                  </span>
                  {esEditable && (
                    <button onClick={() => router.push(`/perfil/pedidos/${p.idPedido}`)}
                      className="text-xs text-amber-600 underline hover:text-amber-800">
                      Editar
                    </button>
                  )}
                  {esCancelable && (
                    <button onClick={() => handleCancelar(p.idPedido)}
                ...
                <div className="mt-2 text-sm space-y-2">
                  ...
                  {p.historial && (
                    <div className="border-t pt-2 mt-2">
                      <p className="font-medium text-xs uppercase tracking-wide text-stone-400 mb-1">Historial de estado</p>
                      {p.historial.map((h) => (
