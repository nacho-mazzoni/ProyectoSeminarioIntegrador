...
export default function EditarProductoPage() {
  ...
  useEffect(() => {
    api.auth.me()
      .then((u) => {
        ...
        setAutorizado(true)
        Promise.all([
          api.productos.obtener(Number(id)),
          api.categorias.listar(),
        ]).then(([p, cats]) => {
          setNombre(p.nombre)
          setIdCategoria(String(p.categoria.idCategoria))
          setPrecioBase(String(p.precioBase))
          setStockEnvases(String(p.stockEnvases))
          setMaxSabores(String(p.maxSabores))
          setCategorias(cats)
        })
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("")
    try {
      await api.admin.productos.actualizar(Number(id), {
        nombre,
  ...
  if (ok)
    return (
      <div className="max-w-lg mx-auto mt-16 px-4 text-center">
        <p className="text-green-600 font-semibold">Producto actualizado correctamente</p>
